...
pub fn sentence() -> String {
    ...
    format!("{}{}{}", first_char, rest, punctuation)
}
...
pub fn words(count: usize, common: bool) -> String {
    ...
    let mut word_list: Vec<&str> = if common {
        COMMON_WORDS.to_vec()
    } else {
        Vec::new()
    };
    ...
    if remaining <= word_list.len() {
        word_list.truncate(remaining);
    } else {
        remaining -= word_list.len();
