...
def test_add_no_argument(assert_parse_error):
    template = "{{ foo|center }}"
    django_message = snapshot("center requires 2 arguments, 1 provided")
...
def test_center_argument_int_bigger_than_isize_max(assert_render_error):
    django_message = snapshot("Python int too large to convert to C ssize_t")
    rusty_message = snapshot("""\
...
def test_center_argument_string(assert_render_error):
    django_message = snapshot("invalid literal for int() with base 10: 'foo'")
    rusty_message = snapshot("""\
